...
The blocklist is a sorted, deduplicated store in PSRAM, sized at boot in `main/app_main.c`:
**up to ~200,000 domains** (`INDEX_CAP`) or **~5.44 MB of domain text** (`BLOB_CAP` = 5 MiB +
200 KiB), whichever fills first — roughly **6.2 MB of the 8 MB PSRAM** (index at 4 B/entry + the
blob). That covers the big curated HOSTS lists (StevenBlack ≈ 150 k) with headroom; it is
deliberately **not** Pi-hole-on-a-server scale. On overflow the extra entry is dropped and a
`truncated` counter increments — bounded and observable (principle II), never a crash.

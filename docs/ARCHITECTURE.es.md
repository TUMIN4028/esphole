...
La lista es un store ordenado y deduplicado en PSRAM, dimensionado al arranque en
`main/app_main.c`: **hasta ~200 000 dominios** (`INDEX_CAP`) o **~5,44 MB de texto de dominios**
(`BLOB_CAP` = 5 MiB + 200 KiB), lo que se llene primero — unos **6,2 MB de los 8 MB de PSRAM**
(índice a 4 B/entrada + el blob). Cubre las grandes listas HOSTS curadas (StevenBlack ≈ 150 k) con
margen; deliberadamente **no** es la escala de Pi-hole en un servidor. Al desbordar, la entrada
sobrante se descarta y un contador `truncated` se incrementa — acotado y observable (principio II),
nunca un crash.
